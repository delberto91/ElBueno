...
//    [self backButtonSetup];
    ...
    UIImageView *imguser = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"bg_tienda"]];
    [self.tableView setBackgroundView:imguser];
...
-(void)customizeTitleView {
    ...
    [view.backImage setHidden:YES];
    self.usernameButton = view.usernameButton;
    self.statusLabel = view.statusLabel;
    self.activityIndicator = view.activityIndicator;
    UITapGestureRecognizer *singleTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(writeToAction:)];
    singleTap.numberOfTapsRequired = 1;
    [view.createChatImage setUserInteractionEnabled:YES];
    [view.createChatImage addGestureRecognizer:singleTap];
    
    
    UIImage *newImage = [view.createChatImage.image imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
    UIGraphicsBeginImageContextWithOptions(view.createChatImage.image.size, NO, view.createChatImage.image.scale);
    [[UIColor whiteColor] set];
    [newImage drawInRect:CGRectMake(0, 0, view.createChatImage.image.size.width, newImage.size.height)];
    newImage = UIGraphicsGetImageFromCurrentImageContext();
    UIGraphicsEndImageContext();
    
    view.createChatImage.image = newImage;
...
//        }
        [ChatUIManager showNotificationWithMessage:conversation.last_message_text image:conversation.conversationId sender:conversation.conversWith senderFullname:conversation.conversWith_fullname];
    }
}
...
-(void)update_unread {
    ...
    if(self.unread_count == 0)
        ...
        [[self navigationController] tabBarItem].badgeValue = [NSString stringWithFormat: @"%d", self.unread_count];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    if (section == 0) {
        ...
        if (conversations && conversations.count > 0) {
            ...
            return [[[[ChatManager getInstance] groupsHandler].allGroups allKeys] count];
...
//            NSLog(@"Conversation.sender %@", conversation.sender);
            ...
            if ([[[[ChatManager getInstance] groupsHandler].allGroups allKeys] count] >0) {
                
                ChatGroup *group =[[ChatManager getInstance] groupById: [[[ChatManager getInstance] groupsHandler].allGroups allKeys][indexPath.row]];
                cell = [CellConfigurator configureGroupConversationCellByGroup:group tableView:tableView indexPath:indexPath conversationsVC:self];
            } else {
                cell = [tableView dequeueReusableCellWithIdentifier:messageCellName forIndexPath:indexPath];
                UILabel *message1 = (UILabel *)[cell viewWithTag:50];
                message1.text = [ChatLocal translate:@"NoConversationsYet"];
...
- (void)tableView:(UITableView *)_tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //MyComunityDescription
        self.typeView = 1;
    [self performSegueWithIdentifier:@"MyComunityDescription" sender:self];
    }else{
        self.typeView = 0;
        self.selectedGroupId = [[[ChatManager getInstance] groupsHandler].allGroups allKeys][indexPath.row];
        [self performSegueWithIdentifier:@"MyComunityDescription" sender:self];
    }
}
-(void)openDescription:(id)sender
{
    ...
    self.selectedGroupId = [[[ChatManager getInstance] groupsHandler].allGroups allKeys][myButton.tag];
    [self performSegueWithIdentifier:@"MyComunityDescription" sender:self];
...
-(void)openChat:(id)sender
{
    ...
    NSArray *conversations = self.conversationsHandler.conversations;
    ChatConversation *selectedConversation = (ChatConversation *)[conversations objectAtIndex:myButton.tag];
    self.selectedConversationId = selectedConversation.conversationId;
    if (selectedConversation.isDirect) {
        self.selectedRecipientId = selectedConversation.conversWith;
        self.selectedRecipientFullname = selectedConversation.conversWith_fullname;
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    }else if ([[segue identifier] isEqualToString:@"MyComunityDescription"]) {
        MyComunityDescriptionVC *vc = (MyComunityDescriptionVC *)[segue destinationViewController];
        
        ChatGroup *groupLocal = [[ChatManager getInstance] groupById:self.selectedGroupId];
        
        vc.typeView = self.typeView;
        vc.nameText = groupLocal.name;
        vc.descriptionText = groupLocal.descripcion;
        vc.userCountText = [NSString stringWithFormat:@"%lu",(unsigned long)groupLocal.members.count];
        vc.selectedGroupId = self.selectedGroupId;
...
-(void)search:(NSString *)text {
    ...
    [db searchContactsByEmailSynchronized:text completion:^(NSArray<ChatUser *> *users) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if([users count] > 0){
                [self openConversationWithUser:users[0]];
            }
        });
    }];
}
...
- (IBAction)swap:(id)sender {
    ...
    self.selectedValue = control.selectedSegmentIndex;
    [[self tableView] reloadData];
}
