...
-(void)searchContactsByEmailSynchronized:(NSString *)searchString completion:(void (^)(NSArray<ChatUser *> *))callback {
    dispatch_async(serialDatabaseQueue, ^{
        NSMutableArray<ChatUser *> *contacts = [[NSMutableArray alloc] init];
        //        const char *dbpath = [databasePath UTF8String];
        //        if (sqlite3_open(dbpath, &database) == SQLITE_OK) {
        NSString *querySQL = [NSString stringWithFormat:
                              @"%@ WHERE email LIKE \"%%%@%%\" ORDER BY email",SELECT_FROM_CONTACTS_STATEMENT, searchString]; //  LIMIT 50
        const char *query_stmt = [querySQL UTF8String];
        if (sqlite3_prepare_v2(database, query_stmt, -1, &statement, NULL) == SQLITE_OK) {
            while (sqlite3_step(statement) == SQLITE_ROW) {
                ChatUser *contact = [self contactFromStatement:statement];
...
-(void)searchContactsByUIDSynchronized:(NSString *)searchString completion:(void (^)(NSArray<ChatUser *> *))callback {
    dispatch_async(serialDatabaseQueue, ^{
        NSMutableArray<ChatUser *> *contacts = [[NSMutableArray alloc] init];
        //        const char *dbpath = [databasePath UTF8String];
        //        if (sqlite3_open(dbpath, &database) == SQLITE_OK) {
        NSString *querySQL = [NSString stringWithFormat:
                              @"%@ WHERE contactId LIKE \"%%%@%%\" ORDER BY contactId",SELECT_FROM_CONTACTS_STATEMENT, searchString]; //  LIMIT 50
        const char *query_stmt = [querySQL UTF8String];
        if (sqlite3_prepare_v2(database, query_stmt, -1, &statement, NULL) == SQLITE_OK) {
            while (sqlite3_step(statement) == SQLITE_ROW) {
                ChatUser *contact = [self contactFromStatement:statement];
...
-(ChatUser *)contactFromStatement:(sqlite3_stmt *)statement {
    ...
    if (_imageurl) {
        imageurl = [[NSString alloc] initWithUTF8String:_imageurl];
    }
